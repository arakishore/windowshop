...
@section('breadcrumb')
    <x-page-header
        ...
        :breadcrumbs="['Merchant' => route('merchant.dashboard'), 'Postal Code Restrictions' => null]"
        :action-url="route('merchant.postal-code-restrictions.create')"
...
@section('content')
    ...
    <div class="alert alert-info">
        <span class="fw-semibold">Shop:</span> {{ app(\App\Services\Merchant\MerchantShopContextService::class)->label($shop) }}
    </div>

    <div class="card">
        <div class="card-header d-flex align-items-center justify-content-between">
            <h5 class="mb-0">Shop Restriction List</h5>
            <a href="#merchant-postal-restriction-filter-collapse" class="text-body collapsed merchant-postal-restriction-filter-toggle" data-bs-toggle="collapse" aria-expanded="false" aria-controls="merchant-postal-restriction-filter-collapse">
                <i class="ph-arrow-circle-down"></i>
            </a>
        </div>
        <div class="collapse {{ $hasFilters ? 'show' : '' }}" id="merchant-postal-restriction-filter-collapse">
            <div class="card-body border-bottom">
                <form method="GET" action="{{ route('merchant.postal-code-restrictions.index') }}" class="row g-3 align-items-end">
                    <div class="col-md-3">
                    ...
                    <div class="col-md-3 d-flex gap-2">
                        ...
                        <a href="{{ route('merchant.postal-code-restrictions.index') }}" class="btn btn-light">Reset</a>
                    </div>
                </form>
            </div>
        </div>
        ...
        @if($restrictions->isEmpty())
            <x-empty-state icon="ph-map-pin-slash" title="No restrictions found" message="Create a shop restriction or adjust the current filters." />
        @else
                            ...
                            <tr>
                                ...
                                <td class="text-center">
                                    <div class="list-icons justify-content-center">
                                        @if($restriction->trashed())
                                            <form method="POST" action="{{ route('merchant.postal-code-restrictions.restore', $restriction) }}" class="d-inline js-confirm-form">
                                                @csrf
                                        ...
                                        @else
                                            <a href="{{ route('merchant.postal-code-restrictions.edit', $restriction) }}" class="list-icons-item text-primary" data-bs-popup="tooltip" title="Edit"><i class="ph-pencil-simple"></i></a>
                                            <form method="POST" action="{{ route('merchant.postal-code-restrictions.destroy', $restriction) }}" class="d-inline js-confirm-form">
                                                @csrf
                                                @method('DELETE')
...
@push('styles')
    <style>
        .merchant-postal-restriction-filter-toggle i { display: inline-block; transition: transform 0.2s ease-in-out; }
        .merchant-postal-restriction-filter-toggle:not(.collapsed) i { transform: rotate(180deg); }
    </style>
@endpush

@push('scripts')
    @include('merchant.postal-code-restrictions.partials.confirm-script')
@endpush
